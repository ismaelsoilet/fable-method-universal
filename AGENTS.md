...
> Portable version for any coding agent or harness (Claude Code, Cursor, Codex, Aider, Gemini CLI, Antigravity CLI, etc.). Identical method to SKILL.md; paste this file into your agent instructions or drop it at your repo root as AGENTS.md.
...
Deeper material loads on demand: `.fable-method/references/failure-modes.md` (symptom to step map for 18 common agent failures), `.fable-method/references/examples.md` (full worked examples for every ask shape), `.fable-method/references/domains/` (domain adapters for non-code work: marketing, research, data analysis, business/ops, finance, legal, design, devops/infrastructure; an adapter changes only the nouns, never the loop, and its minimum evidence set is binding).

**Domain adapters.** Coding is the default domain. If the task is marketing/content, research/reporting, data analysis, business/ops, finance, legal/compliance, design/UX, or devops/infrastructure (IaC, pipelines, deploys, monitoring: script logic stays coding; live-state changes route here), read the matching file in `.fable-method/references/domains/` before Step 2. An adapter changes only the nouns, never the loop: what counts as evidence, who the authority is, what verification by observation means, and what the frauds are. Its **minimum evidence set is binding**: those items must actually be opened before acting, every time. Research is never optional; the adapter defines how much is enough. Sales/support tasks use marketing plus business-ops; education content uses research. Medical and clinical work has no adapter on purpose: it needs qualified review, not a checklist; say so when asked.
...
5. **Track multi-part work.** Any task with 3 or more heterogeneous steps, or more than ~5 similar items, gets a written checklist first (maintain a markdown checklist in context if no todo tool is available). Tick items as they complete; audit the list against the original ask before reporting.
...
**audit** - grade the most recent completed piece of work in this conversation against the loop. For each step, mark it followed, skipped, or faked (claimed without observation). For every skip or fake, name the concrete risk it created; `.fable-method/references/failure-modes.md` maps symptoms to steps. Deliver a short table plus the single highest-value fix, and apply that fix only if the user asks.
